...
class PrescriptionDAO:
    ...
    def insertPrescription(self, prescription, assistantid, doctorid, dateofupload, patientid, recordno):
        try:
            connection_url = "host=%s, port=%s, dbname=%s user=%s password=%s" % (
                pg_config['host'], pg_config['port'], pg_config['dbname'], pg_config['user'], pg_config['passwd'])
            self.conn = psycopg2._connect(connection_url)
            ...
            try:
                ...
                cursor.execute(query, (prescription, assistantid, doctorid, dateofupload, patientid, recordno,))
