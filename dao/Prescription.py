...
class PrescriptionDAO:
    ...
    def insertPrescription(self, prescriptionlink, assistantusername, doctorusername, dateofupload, patientid, recordno):
        try:
            connection_url = "host=%s, port=%s, dbname=%s user=%s password=%s" % (
                pg_config['host'], pg_config['port'], pg_config['dbname'], pg_config['user'], pg_config['passwd'])
            self.conn = psycopg2._connect(connection_url)
            ...
            try:
                cursor = self.conn.cursor()
                query = "insert into prescriptions (prescription, assistantusername, doctorusername, dateofupload, " \
                        "patientid, recordno) " \
                        "values (%s,%s,%s,%s,%s,%s) " \
                        "returning prescriptionid;"
                cursor.execute(query, (prescriptionlink, assistantusername, doctorusername, dateofupload, patientid, recordno,))
